...
function GameScreen({ setUserTime }: Props) {
  ...
  const audioShouldPlay = useAppSelector((state) => state.game.audioShouldPlay);
  const liveGameData = useAppSelector((state) => state.game.liveGameData);
  ...
  const eraserUsedAlreadyRef = useRef(false);
  ...
  const fetchGame = useCallback(async () => {
    ...
    const timeoutPromise = new Promise<never>((_, reject) =>
      setTimeout(
        () => reject(new Error("Request timed out, Click Start Game Now")),
        10000
      )
    );

    try {
      const res = await Promise.race([GameApi.fetchNextGame(), timeoutPromise]);
  ...
  //       const delta = timestamp - startTime;
  //       startTime = timestamp;
  //       //   setTotalTimeUsed((prev) => prev + delta);
  //       totalTimeRef.current += delta;
  ...
  const handleNextQuestion = async () => {
    ...
    const totalTimeFormatted = formatTime(totalTimeRef.current);
  ...
  const handleOptionClick = async (option: string) => {
    ...
    if (isCorrect) {
      ...
      eraserUsedAlreadyRef.current = true;
